...
### 성능 최적화 및 UI 개선 (2026-03-31)

| 항목 | 내용 | 결과 |
| --- | --- | --- |
| BGM preload 최적화 | `preload='auto'` → `'metadata'` 변경 | 44MB 즉시 다운로드 방지, 이미지 로딩과 네트워크 경합 해소 |
| 이미지 리사이즈 + WebP 재생성 | 520px 리사이즈 + quality 72→60 | 15.2MB → 3.1MB (80% 절감), `scripts/generate-webp.js` 수정 |
| 프리로드 중복 요청 제거 | WebP + JPG 동시 프리로드 → WebP 단독 | 초기 프리로드 요청 수 절반 감소 |
| 라우트 코드 스플리팅 | 정적 import → 동적 import | Vite 빌드 시 뷰별 청크 분리, 초기 번들 파싱 시간 단축 |
| 축하 화면 날짜 동적 처리 | `'2026.04.05'` 하드코딩 제거 | 실행 시점의 연월일(YYYY.MM.DD)을 동적으로 표시 |
| 카드 이미지 블러 배경 적용 | 동일 이미지 블러 레이어를 배경에 삽입 | 세로 사진 양 옆 빈 공간 자연스럽게 채움, max-height: 70vh로 하단 잘림 해소 |
| 21번 카드 날짜 누락 보완 | `TextGuide.md` 날짜 입력 후 `generate:data` 재실행 | memory-021 날짜 `2023-08-27` 반영 |
