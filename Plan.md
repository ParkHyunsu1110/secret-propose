...
- 타인에게 공개할 때는 `/share` 진입 후 동일 플로우. 카드만 `shareMemories.js`에 정의된 memory id만 순서대로 표시. 편지·프로포즈·축하는 본편과 동일.
...
- **카드 전환**: 버튼 클릭 (이전/다음)
- **카드 프레임 구성**:
  - 이미지 (사진 영역만 표시, 날짜/제목/설명 숨김)
  - ~~타이틀 (날짜 + 장소)~~ (현재 미표시)
  - ~~회고 글 (그 당시에 대한 이야기)~~ (현재 미표시)
- 추억할 내용마다 프레임 1개씩 생성
...
9. [x] 카드 1개 + 사진 87장 구조 (버튼으로 사진 넘기기)
